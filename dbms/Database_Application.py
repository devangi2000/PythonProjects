...
#setting logo and title
        #self.canv = Canvas(root, width=80, height=80, bg='white')
        ...
        self.label1 = Label(font=('arial', 15, 'bold'), text='School Portal', fg='cyan')
...
#MESSAGE DISPLAY
        self.message = Label(text='',fg='cyan')
...
#ADD A NEW Record
    def validation(self):
        return len(self.firstname.get()) !=0  and len(self.lastname.get()) !=0 and len(self.username.get()) !=0 and len(self.subject.get()) !=0 and len(self.age.get()) !=0 and len(self.email.get()) !=0
    ...
    def add_record(self):
        if self.validation():
            ...
            self.message['text'] = 'Record {} {} inserted'.format(self.firstname.get(), self.lastname.get())
...
#CLEAR Fields
            ...
            self.message['text'] = 'Fields not completed! Please fill all required fields.'
    ...
    def dele(self):
        de = tkinter.messagebox.askquestion('Delete record', 'Do you want to delete the record?')
        if de == 'yes':
            self.delete_record()
#EDIT records
    def edit_box(self):
    ...
    def edit_record(self, new_fname, fname, new_lname, lname, new_uname, uname, new_email, email, new_subject, subject, new_age, age):
        query = 'UPDATE details SET Firstname=?, Lastname=?, Username=?, Email=?, Subject=?, Age=? WHERE Firstname=? AND Lastname=? AND Username=? AND Email=? AND Subject=? AND Age=?'
        ...
        self.message['text'] = '{} details have been changed to {}'.format(fname, new_fname)
        self.viewing_records()
    ...
    def exit(self):
        exit = tkinter.messagebox.askquestion('Exit Application?', 'Do you want to close the application?')
        if exit == 'yes':
            root.destroy()
